...
const categories = ["All", "Websites"] as const

const getPreviewImage = (project: any) => {
  const src = project?.images?.[0] || project?.image || "/placeholder.svg"
  const withSlash = src.startsWith("/") ? src : `/${src}`
  return withSlash.replace(/ /g, "%20")
}

const getSecondaryPreviewImage = (project: any) => {
...
export default function WorkPage() {
  ...
  const filteredProjects = useMemo(() => {
    ...
    return workProjects.filter((p) => p.category === activeCategory)
  }, [activeCategory])
    ...
    <main className="relative min-h-screen pt-28 md:pt-32 pb-24 px-6 md:px-10 lg:px-16 bg-[#0D0D0D] overflow-hidden">
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-10 mb-16">
          ...
          {/* Filters */}
          <div className="flex flex-wrap gap-3 lg:static sticky top-20 z-20 backdrop-blur-md bg-black/30 py-3 px-2 -mx-2 rounded-xl border border-white/10 lg:border-0 lg:bg-transparent lg:backdrop-blur-0 lg:py-0 lg:px-0 lg:mx-0">
            {categories.map((cat) => {
              ...
              return (
                <button
                  ...
                  className={[
                    "text-[10px] md:text-xs uppercase tracking-[0.3em] px-4 md:px-5 py-2.5 md:py-3 border rounded-lg transition-all duration-300",
                    "focus:outline-none focus:ring-1 focus:ring-white/40",
                    isActive
                      ? "border-white text-white bg-white/8"
                      : "border-white/15 text-white/55 hover:text-white hover:border-white/50 hover:bg-white/[0.04]",
                  ].join(" ")}
          ...
          <AnimatePresence mode="popLayout">
            {filteredProjects.map((project) => (
              <motion.article
                ...
                key={project.slug}
                className="group relative aspect-[4/5] overflow-hidden bg-neutral-950 rounded-2xl border border-white/10 hover:border-white/25 transition-all cursor-pointer active:scale-[0.99] active:border-white/40 focus-within:border-white/35"
              >
                <Link href={`/work/${project.slug}`} aria-label={`View case study: ${project.title}`} className="block w-full h-full">
                  ...
                  <div className="absolute bottom-0 left-0 w-full p-6 md:p-7 translate-y-3 group-hover:translate-y-0 transition-transform duration-500">
                    <div className="flex justify-between items-end border-b border-white/15 pb-3 mb-2">
                      ...
                      <div className="flex items-end gap-2">
                        {project.type && (
                          <span className="text-[9px] md:text-[10px] uppercase tracking-[0.25em] text-white/50 bg-white/5 border border-white/10 px-2 py-1 rounded-md">
                            {project.type}
                          </span>
                        )}
                        <span className="text-[10px] md:text-[11px] text-white/55 tracking-[0.2em] uppercase">
                          {project.year}
                        </span>
                      </div>
                    </div>

                    <p className="text-[10px] md:text-[11px] uppercase tracking-[0.3em] font-semibold">
                  ...
                  {/* Sharp hover frame */}
                  <div className="absolute inset-3 border border-white/0 group-hover:border-white/20 rounded-xl transition-all duration-500 pointer-events-none shadow-[0_0_0_0_rgba(255,255,255,0)] group-hover:shadow-[0_0_30px_rgba(255,255,255,0.08)]" />
                </Link>
              </motion.article>
            ))}
            {filteredProjects.length === 0 && (
              <div className="col-span-full border border-white/10 bg-neutral-950 rounded-2xl p-10 md:p-14 text-center">
                ...
                <button
                  ...
                  className="text-[10px] md:text-xs uppercase tracking-[0.3em] px-5 py-3 border rounded-lg transition-all duration-300 border-white/20 text-white/70 hover:text-white hover:border-white/50 hover:bg-white/[0.04]"
                >
                  Back to All
                </button>
              </div>
